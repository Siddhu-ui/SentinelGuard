...
import {AlertTriangle,Copy,Download,X,ChevronDown,ShieldAlert} from 'lucide-react';
import {Scan} from './App';

export default function Result({scan,token,close,onQuarantine}:{scan:Scan;token:string;close:()=>void;onQuarantine?:(scan:Scan)=>Promise<void>}){
  const [downloading,setDownloading]=useState(false);
  const [copied,setCopied]=useState(false);
  const [quarantining,setQuarantining]=useState(false);
  const [quarantined,setQuarantined]=useState(false);
  const [qError,setQError]=useState('');
  // Three distinct outcomes: failed analysis, completed with findings, completed clean.
  const analysisFailed=scan.analysis_status==='failed';

  const quarantine=async()=>{
    if(!onQuarantine||quarantined)return;
    setQuarantining(true);setQError('');
    try{
      await onQuarantine(scan);
      setQuarantined(true);
    }catch(e:any){setQError(e?.message||'Quarantine failed.')}
      ...
      <section className="modal-card">
        ...
        {analysisFailed ? (
          <div className="analysis-failed" role="alert">
            ...
            <div>
              <b>Analysis failed</b>
              <p>{scan.analysis_error || 'The analysis engine could not process this file. Results are unavailable for this upload.'}</p>
              <small>The scan record was saved, but no findings were produced and the risk score is unavailable.</small>
            </div>
          </div>
        ) : (
        ...
        <div className="details">
          ...
          <div className="detail">
            <b>SHA-256</b>
            <code>{scan.sha256 || 'unavailable (analysis failed)'}</code>
            {scan.sha256&&<button className="iconbtn" onClick={copy} title="Copy"><Copy size={14}/>{copied?'Copied':'Copy'}</button>}
          </div>
          <div className="detail"><b>Detected type</b><span>{scan.mime_type}</span></div>
          {analysisFailed
            ? <div className="detail"><b>Entropy</b><span>unavailable (analysis failed)</span></div>
            : <div className="detail"><b>Entropy</b><span>{scan.entropy}/8 <em>({scan.details?.entropy_category})</em></span></div>}
        </div>

        {!analysisFailed && scan.details?.analysis_sections?.length > 0 && <section className="analysis-sections">
          ...
          ? scan.threats.map((t:any,i:number)=>(
            <div className="finding" key={i}>
              ...
              <span>
                <b>{t.category}</b>
                <br/>{t.message}
                {t.evidence&&<small className="finding-evidence">Evidence: {t.evidence}</small>}
              </span>
              <em className={'sev '+t.severity}>{t.severity}</em>
            </div>
          ))
          :<div className="finding ok"><AlertTriangle/><span><b>Clean</b><br/>Analysis completed. No suspicious indicators detected.</span></div>
        }

        {!analysisFailed && scan.details?.score_breakdown?.length > 0 && <div className="score-breakdown">
        ...
        {!analysisFailed && scan.details?.hex_preview?.length > 0 && <details className="hex-inspector"><summary><ChevronDown size={15}/> Binary inspector (first 256 bytes)</summary><pre>{scan.details.hex_preview.map((row:any)=>`${row.offset}  ${row.hex.padEnd(47,' ')}  ${row.ascii}`).join('\n')}</pre></details>}
        ...
        <div className="result-actions">
          ...
          {onQuarantine&&(
            quarantined
              ? <button className="ghost quarantine-done" disabled><ShieldAlert size={15}/> Moved to Quarantine — manage it under Quarantine</button>
              : <button className="ghost danger-ghost" onClick={quarantine} disabled={quarantining||analysisFailed} title="Isolate this file: it is removed from normal storage and cannot be opened">
                  <ShieldAlert size={15}/> {quarantining?'Moving…':'Quarantine this file'}
                </button>
          )}
